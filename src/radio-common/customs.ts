...
export default class RadioFilter {
    ...
    public async indexerCheck(client: Client, address: string){
        ...
        if (senderStake < this.minStakeReq){
            console.error(`Identity does not satisfy the minimum indexer stake, message discarded.`, {
                senderStake,
    ...
    public async disputeStatusCheck(client:Client, address: string){
        ...
        return senderDisputes.reduce((slashedRecord, dispute) => slashedRecord + Number(dispute.tokensSlashed), 0)
    }
    ...
    public async poiMsgChecks(client:Client, sender:string, timestamp:number){
        ...
        if ((senderStake == 0) || (!this.replyThreshold(timestamp)) || (tokensSlashed > 0)) {
