...
const setup = async () => {
  ...
  db.run(
    "CREATE TABLE IF NOT EXISTS poi_crosschecker (subgraph VARCHAR, block BIGINT, nPOI VARCHAR, operator VARCHAR, stake_weight BIGINT, nonce BIGINT)"
  );
  db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?)", ["Qmaaa", 0, "0x0", "operator1", 1, Date.now()]);
};
...
describe("Radio helpers", () => {
  ...
  describe("Process attestations", () => {
    ...
    test("single local record", async () => {
      ...
      db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?, ?)", ["Qmaaa", 1, "0x0", "operator2", 1, Date.now()]);
      ...
      db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?, ?)", ["Qmaaa", 0, "0x0", "operator2", 1, Date.now()]);
    ...
    test("add attacks", async () => {
      // different block
      db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?, ?)", ["Qmaaa", 1, "0x1", "operator2", 1, Date.now()]);
      ...
      db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?, ?)", ["Qmaaa", 0, "0x1", "operator2", 1, Date.now()]);
      ...
      db.run("INSERT INTO poi_crosschecker VALUES (?, ?, ?, ?, ?, ?)", ["Qmaaa", 0, "0x1", "operator2", 2, Date.now()]);
